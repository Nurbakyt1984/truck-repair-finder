...
async def photo_or_text(u,c):
    ...
    if u.message.photo:
        try:
            ...
            await u.message.reply_text('📷 Данные распознаны. Сейчас проверю адрес.')
        except Exception:
            logger.exception('OCR failed'); await u.message.reply_text('Не удалось распознать фото автоматически. Нажмите «✍️ Ввести вручную» или отправьте данные одной формой текстом.'); return INPUT
    else:
